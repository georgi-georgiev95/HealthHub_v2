...
describe('Register Component', () => {
    ...
    it('should disply error if password is more than 20 characters', async () => {
        ...
        await act(async () => {
            ...
            fireEvent.change(passwordInput[1], { target: { value: '123456789012345678901' } });
            fireEvent.blur(passwordInput[0]);
            fireEvent.blur(passwordInput[1]);
        });
    
        expect(screen.getByText('Password must be at most 20 characters long!')).toBeInTheDocument();
    });
    ...
    it('should disply error if username includes whitespaces', async () => {
        ...
        expect(screen.getByText('Username can not include whitespaces!')).toBeInTheDocument();
    });
});
